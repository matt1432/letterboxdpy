...
class Scraper:
    ...
    BLOCK_KEYWORDS: ClassVar[list[str]] = [
        "cloudflare",
        "permission denied",
        "security challenge",
        "captcha",
    ...
    ERR_FORBIDDEN_FALLBACK = (
        "Forbidden: Access denied. Your network might be restricted or IP is flagged."
    )
    ...
    def _fetch(cls, url: str) -> requests.Response:
        ...
        for attempt in range(max_retries):
            try:
                ...
                current_timeout: tuple[int, int] = (
                    cls.timeout[0] + attempt * 2,
                    cls.timeout[1] + attempt * 5,
                )
                ...
                if response.status_code == 403 and attempt < max_retries - 1:
                    wait = 2 * (attempt + 1) + random.random()  # noqa: S311
                    time.sleep(wait)
                    continue
                ...
                if attempt < max_retries - 1:
                    ...
                    wait = 3 * (attempt + 1) + random.random()  # noqa: S311
                    time.sleep(wait)
                    continue
                break
        ...
        if last_exception:
            raise PageLoadError(
                url,
                f"Network error ({type(last_exception).__name__}): {last_exception!s}",
            )
    ...
    def _check_for_errors(cls, url: str, response: requests.Response) -> None:
        ...
        if response.status_code != 200:
            ...
            if response.status_code == 403:
                ...
                is_blocked = (
                    error_message == cls.ERR_VPN_BLOCK
                    or error_message.startswith("Forbidden:")
                )
